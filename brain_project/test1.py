...
# Assign each region to one of the 7 functional networks (sizes chosen to sum to 65)
network_sizes = [10, 8, 12, 9, 11, 7, 8]
region_to_network = np.repeat(np.arange(n_networks), network_sizes)

# -------------------------------
# Build a base correlation matrix with block structure
# -------------------------------
...
# Ensure positive definiteness (add a small ridge if needed)
...
# Center the PMAT scores for later effect modulation
pmat_cr_centered = pmat_cr - np.mean(pmat_cr)

# -------------------------------
# Generate subject‑specific correlation matrices
# -------------------------------
all_corr = []
for i in range(n_subjects):
    ...
    # Introduce a systematic effect: edges within network 0 and network 1
    ...
    # Add random noise using Wishart distribution to create realistic variability
    # Use the current correlation as the scale matrix for a covariance
    ...
    # Draw a covariance matrix from Wishart
...
# -------------------------------
# Flatten the upper triangle (excluding diagonal) of each correlation matrix
# This yields a 2D array of shape (977, n_edges)
...
# Create column names for the edges (e.g., "r0_r1", "r0_r2", ...)
...
print("Synthetic dataset generated and saved as CSV files.")
